...
// Lmu computes the value of the function Lmu(L) = 1/k * X^mu * Tr(L)
func Lmu(mu, k int, L math.Polynomial, galEl, invk *math.ModInt) math.Polynomial {
	return L.Copy().(math.Polynomial).
		Trace(galEl, k).
		RShift(mu).
