...
func TestMulVec(t *testing.T) {
	...
	M := math.NewMatrixFromDimensions(5, 3).Populate(
		func(row int, col int) math.RingElement {
			return math.NewModInt(big.NewInt(int64(col+1)), big.NewInt(100))
		})
	x := math.NewVectorFromSize(3).Populate(
		func(i int) math.RingElement {
			return math.NewModInt(big.NewInt(int64(1)), big.NewInt(100))
		})
	// Expected result
	y := math.NewVectorFromSize(5).Populate(
		func(i int) math.RingElement {
			return math.NewModInt(big.NewInt(int64(6)), big.NewInt(100))
		})
	res := M.MulVec(x)
...
func TestTranspose(t *testing.T) {
	...
	M := math.NewMatrixFromDimensions(4, 3).Populate(
		func(row int, col int) math.RingElement {
			return math.NewModInt(big.NewInt(int64(row+1)), big.NewInt(100))
		})
	M.Transpose()
